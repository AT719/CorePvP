...
class Main extends PluginBase implements Listener {
    ...
    public function onPvP(EntityDamageByEntityEvent $event): void {
        ...
        if ($victim instanceof Player && $attacker instanceof Player) {
            if ($victim->getHealth() - $event->getFinalDamage() <= 0) {
                $this->addStat($attacker, "kills", 1);
                $this->addStat($attacker, "money", 100);
                $this->addStat($attacker, "exp", 50);
                $attacker->sendMessage("§e[ONPU] §fKill! +100 Money");
                $this->updateScoreboard($attacker);
            }
            $name = $victim->getName();
            if (isset($this->playerKit[$name]) && $this->playerKit[$name] === "mikawa") { 
                if ($event->getCause() === EntityDamageEvent::CAUSE_PROJECTILE) { $event->setBaseDamage(0); $event->cancel(); return; } 
            } 
            if (isset($this->activeSkills[$name]["mikawa"]) && $this->activeSkills[$name]["mikawa"] > time()) { 
                if (mt_rand(0, 1) === 0) { $event->cancel(); $victim->sendPopup("§b回避！"); return; } 
            } 
            if (isset($this->activeSkills[$name]["assault"]) && $this->activeSkills[$name]["assault"] > time()) { 
    ...
    private function handleCoreBreak(Player $p, $block, BlockBreakEvent $event): void {
        ...
        if ($myTeam === $targetTeam) { $p->sendMessage("§c味方のコアは攻撃できません！"); return; }
        if ($this->currentPhase === 1) { $p->sendPopup("§cPhase 1: コアはまだ保護されています！"); $this->playSound($p, "note.bass", 1.0, 0.5); return; }
        ...
        if ($currentHP <= 0) { $this->endGame($myTeam); }
    }
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool {
        ...
        switch ($command->getName()) {
            ...
            case "kit": $sender->sendMessage("§cKitコマンドは無効化されています。拠点のネザーゲートに入って変更してください。"); return true;
            case "forcestart":
    ...
    public function sendVoteForm(Player $player): void { 
        $form = new SimpleForm(function (Player $player, $data) { 
            if ($data === null) return; 
            $maps = [0 => "Canyon", 1 => "Valley", 2 => "Coastal", 3 => "Skylands"]; 
            if (isset($maps[$data])) { 
                $this->votes[$player->getName()] = $maps[$data]; 
                $player->sendMessage("§a[ONPU] §f" . $maps[$data] . " に投票しました！"); 
            } 
        }); 
        $form->setTitle("Map Vote"); 
        $form->setContent("プレイしたいマップを選んでください"); 
        $form->addButton("Canyon"); 
        $form->addButton("Valley"); 
        $form->addButton("Coastal"); 
        $form->addButton("Skylands"); 
